...
const evidenceData = {
  photo: {
    ...
    location: "library",
    title: "泛黃的社團合照",
    desc: "照片背面寫著「三年前，廣播社」。最右側的人臉被奇怪地用修正帶塗掉，但制服名牌還露出一個「祐」字。",
  },
  diary: {
    name: "日記",
    icon: "DIARY",
    location: "infirmary",
    title: "保健室借宿日記",
    desc: "日記裡反覆出現一句話：「只要我不回答，大家就能繼續說我不存在。」最後一頁被撕掉。",
  },
  roster: {
    name: "點名簿",
    icon: "LIST",
  ...
  cctv: {
    ...
    title: "舊校舍走廊錄影",
    desc: "錄影時間是失蹤當天 17:42。畫面裡有三名學生把一個書包塞進廣播室置物櫃，隔天才被取出。",
  },
};
...
const locations = [
  {
    ...
    scene: "classroom",
    clue: "roster",
    text: "黑板上還留著放學前的值日生名單。粉筆灰像一層薄霧，點名簿被壓在講桌抽屜最底層。",
  },
  {
    id: "library",
    name: "圖書館",
    scene: "library",
    clue: "photo",
    text: "舊校刊區有一排沒人借的畢業紀念冊。某一頁被摺角，裡面夾著廣播社的合照。",
  },
  {
    id: "infirmary",
    name: "保健室",
  ...
  },
  {
    ...
    scene: "old-building",
    clue: "cctv",
    text: "舊校舍的走廊比傳聞中乾淨，乾淨得像有人定期回來整理。保全室備份碟還能讀取。",
  },
  {
    id: "broadcastRoom",
    name: "廣播室",
...
const dialogues = [
  {
    speaker: "林澄",
    avatar: "澄",
    text: "第七節下課後，雨把操場切成一格一格的灰色。廣播忽然響了，聲音像從很遠的教室傳來。",
  },
  {
    speaker: "廣播",
    avatar: "播",
  ...
  },
  {
    speaker: "林澄",
    avatar: "澄",
    text: "我差點回答。可班導曾說過那句都市傳說：放學後如果聽到廣播點名，千萬不要回答自己的名字。",
  },
  {
    speaker: "許眠",
    avatar: "眠",
    text: "你也聽到了嗎？剛剛點到三號之後，還有一個名字。可是我們班沒有那個人。",
  },
  {
    speaker: "廣播",
    avatar: "播",
  ...
  },
  {
    speaker: "林澄",
    avatar: "澄",
    text: "班上明明只有二十六個人。可所有人都在同一瞬間低下頭，像是這個名字不該被聽見。",
  },
  {
    speaker: "系統",
    avatar: "案",
    text: "CH1 結束。你的目標不是證明鬼存在，而是找出大家為什麼假裝不記得。",
...
const investigationIntro = [
  {
    speaker: "許眠",
    avatar: "眠",
    text: "CH2 初次調查。先找四樣證物：照片、日記、點名簿、監視器紀錄。它們會把同一段被塗掉的記憶拼回來。",
  },
];
...
const rebuttalIntro = [
  {
    speaker: "林澄",
    avatar: "澄",
    text: "證物都到齊了。現在要問的不是「誰失蹤」，而是「誰讓他只能用失蹤被記住」。",
  },
];
...
function setDialogue(line) {
  ...
  el.avatarInitial.textContent = line.avatar;
  el.dialogueText.classList.remove("glitch");
  void el.dialogueText.offsetWidth;
  if (line.speaker === "廣播") el.dialogueText.classList.add("glitch");
  typeText(line.text);
}

function currentDialogueList() {
  if (state.mode === "investigationIntro") return investigationIntro;
  if (state.mode === "rebuttalIntro") return rebuttalIntro;
  return dialogues;
}
...
function advanceDialogue() {
  ...
  const line = list[state.dialogueIndex];
  if (line?.action === "startInvestigation") {
    enterInvestigationIntro();
    return;
  }

  state.dialogueIndex += 1;
  if (state.dialogueIndex >= list.length) {
    if (state.mode === "investigationIntro") enterExploreMode();
    else if (state.mode === "rebuttalIntro") enterDeductionMode();
    else state.dialogueIndex = list.length - 1;
    return;
  }
...
function enterInvestigationIntro() {
  ...
  setDialogue(investigationIntro[0]);
}
...
function renderEvidence() {
  ...
  el.progressText.textContent = `線索 ${state.evidence.size} / 4`;
...
function checkAllEvidence() {
  if (state.evidence.size >= Object.keys(evidenceData).length) {
...
function refute() {
  ...
  if (testimony.answer === evidenceId) {
    ...
    el.deductionResult.textContent = "這項證物還不能直接推翻該證詞。換一個角度：證詞中哪一句話最怕被看見？";
  }
}
...
function finishChapterTwo() {
  ...
  typeText("所有矛盾都指向同一件事：陳祐安沒有被鬼帶走。他是被班級、社團和紀錄一起推到記憶外面。下一章，該輪到真正的當事人開口。");
}
...
function toggleAudio() {
  ...
  const volume = state.audioStarted ? 0.12 : 0;
  const humVolume = state.audioStarted ? 0.018 : 0;
  state.audioNodes.gain.gain.setTargetAtTime(volume, state.audioNodes.ctx.currentTime, 0.05);
  state.audioNodes.humGain.gain.setTargetAtTime(humVolume, state.audioNodes.ctx.currentTime, 0.05);
...
function resetGame() {
  ...
  state.solvedContradictions.clear();
